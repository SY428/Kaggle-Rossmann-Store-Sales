...
MaxGapDays <- as.numeric()
GapStart <- as.Date("2013-01-01")
...
for (i in 1:max(testStore$Store)) {
  if (i %in% unique(testStore$Store)) {
    ...
    if ((max(trainStore_i$Date) - min(trainStore_i$Date)) > length(trainStore_i$Date)) {
      ...
      MaxGapDays <- c(MaxGapDays, -min(diff(trainStore_i$Date)))
      GapStart <- c(GapStart, trainStore_i$Date[which(diff(trainStore_i$Date) == min(diff(trainStore_i$Date)))+1])
    }
  }
}

MissingDays.df <- data.frame(Store = StoresWithMissingDays, Days = MissingNumDays, Max = MaxGapDays, GapStart = GapStart[-1])
...
ClosedNumDays <- as.numeric()
ClosedStart <- as.Date("2013-01-01")
ClosedStoreThreeToFive <- as.numeric()

for (i in min(ThreeToFiveDaysClosed):max(ThreeToFiveDaysClosed)) {
  if (i %in% ThreeToFiveDaysClosed) {
    trainStore_i <- subset(trainStore, trainStore$Store == i)
    trainStoreOpen_i <- subset(trainStore_i, trainStore_i$Open == 1)
    
    len <- length(trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -3)+1])
    if (len > 0) {
      for (k in 1:len) {
        ClosedStoreThreeToFive <- c(ClosedStoreThreeToFive, i)
        ClosedStart <- c(ClosedStart, trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -3)+1][k])
        ClosedNumDays <- c(ClosedNumDays, trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -3)][k] - trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -3)+1][k])
      }
    } else { # Special Cases
      ClosedStoreThreeToFive <- c(ClosedStoreThreeToFive, i)
      if(trainStore_i$Date[1] == trainStoreOpen_i$Date[1]) { # At the beginning
        ClosedStart <- c(ClosedStart, trainStore_i$Date[length(trainStore_i$Date)])
        ClosedNumDays <- c(ClosedNumDays, trainStoreOpen_i$Date[length(trainStoreOpen_i$Date)] - trainStore_i$Date[length(trainStore_i$Date)])
      } else { # At the end
        ClosedStart <- c(ClosedStart, trainStoreOpen_i$Date[length(trainStoreOpen_i$Date)])
        ClosedNumDays <- c(ClosedNumDays, trainStore_i$Date[length(trainStore_i$Date)] - trainStoreOpen_i$Date[length(trainStoreOpen_i$Date)])
      }
    }
  }
}

ThreeToFiveDays.df <- data.frame(Store = ClosedStoreThreeToFive, StartDate = ClosedStart[-1], MaxNumDays = ClosedNumDays)

ClosedNumDays <- as.numeric()
ClosedStart <- as.Date("2013-01-01")
ClosedStoreSixToNine <- as.numeric()

for (i in min(SixToNineDaysClosed):max(SixToNineDaysClosed)) {
  if (i %in% SixToNineDaysClosed) {
    trainStore_i <- subset(trainStore, trainStore$Store == i)
    trainStoreOpen_i <- subset(trainStore_i, trainStore_i$Open == 1)
    
    len <- length(trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -6)+1])
    if (len > 0) {
      for (k in 1:len) {
        ClosedStoreSixToNine <- c(ClosedStoreSixToNine, i)
        ClosedStart <- c(ClosedStart, trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -6)+1][k])
        ClosedNumDays <- c(ClosedNumDays, trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -6)][k] - trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -6)+1][k])
      }
    } else { # Special Cases
      ClosedStoreSixToNine <- c(ClosedStoreSixToNine, i)
      if(trainStore_i$Date[1] == trainStoreOpen_i$Date[1]) { # At the beginning
...
SixToNineDays.df <- data.frame(Store = ClosedStoreSixToNine, StartDate = ClosedStart[-1], MaxNumDays = ClosedNumDays)

ClosedNumDays <- as.numeric()
ClosedStart <- as.Date("2013-01-01")
ClosedStoreTenPlus <- as.numeric()

for (i in min(TenOrMoreDays):max(TenOrMoreDays)) {
  if (i %in% TenOrMoreDays) {
    trainStore_i <- subset(trainStore, trainStore$Store == i)
    trainStoreOpen_i <- subset(trainStore_i, trainStore_i$Open == 1)
    
    len <- length(trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -10)+1])
    if (len > 0) {
      for (k in 1:len) {
        ClosedStoreTenPlus <- c(ClosedStoreTenPlus, i)
        ClosedStart <- c(ClosedStart, trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -10)+1][k])
        ClosedNumDays <- c(ClosedNumDays, trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -10)][k] - trainStoreOpen_i$Date[which(diff(trainStoreOpen_i$Date) <= -10)+1][k])
      }
    } else { # Special Cases
      ClosedStoreTenPlus <- c(ClosedStoreTenPlus, i)
      if(trainStore_i$Date[1] == trainStoreOpen_i$Date[1]) { # At the beginning
...
TenOrMoreDays.df <- data.frame(Store = ClosedStoreTenPlus, StartDate = ClosedStart[-1], MaxNumDays = ClosedNumDays)
TenOrMoreDays.df <- subset(TenOrMoreDays.df, TenOrMoreDays.df$MaxNumDays < 180) # Double checked
...
for (i in 1:max(testStore$Store)) {
  if ((i %in% unique(testStore$Store)) & !(i %in% SpecialStores) & !(i %in% MissingDays.df$Store)) {
...
####################################
### Gap Cases: Missing or Vaca Days
####################################

for (i in 1:max(testStore$Store)) {
  if ((i %in% MissingDays.df$Store) | (i %in% TenOrMoreDays.df$Store) | (i %in% SixToNineDays.df$Store)) {
    print(paste("Store: ", i, sep = ""))
    trainStore_i <- subset(trainStoreAvg, trainStoreAvg$Store == i)
    
    section <- sum(MissingDays.df$Store == i) + sum(TenOrMoreDays.df$Store)
    if (section == 1) {
      trainStore_i_0 <- subset(trainStore_i, trainStore_i$Date <= unique(MissingDays.df$GapStart))
      trainStore_i_1 <- subset(trainStore_i, trainStore_i$Date > unique(MissingDays.df$GapStart))
      
      xreg_i_0 <- model.matrix(~ droplevels(as.factor(trainStore_i_0$DayOfWeek)) 
                             + trainStore_i_0$Promo
                             + trainStore_i_0$SchoolHoliday
                             + trainStore_i_0$ClosedDayPrior
                             + trainStore_i_0$ClosingNextDay
                             + trainStore_i_0$PromoWithin3Days
                             + trainStore_i_0$PromoEndedWithin3Days
                             + trainStore_i_0$TimePriorWasSchoolHoliday
                             + trainStore_i_0$DaysWithinSchoolHoliday
                             + trainStore_i_0$Open
      )
      xreg_i_0 <- xreg_i_0[,-1]
      colnames(xreg_i_0) <- c("Mon", "Sat", "Sun", "Thur", "Tues", "Wed", 
                            "Promo", 
                            "SchoolHoliday", 
                            "ClosedDayPrior", 
                            "ClosingNextDay", 
      ...
      xreg_i_1 <- model.matrix(~ droplevels(as.factor(trainStore_i_1$DayOfWeek)) 
                               + trainStore_i_1$Promo
                               + trainStore_i_1$SchoolHoliday
                               + trainStore_i_1$ClosedDayPrior
                               + trainStore_i_1$ClosingNextDay
                               + trainStore_i_1$PromoWithin3Days
                               + trainStore_i_1$PromoEndedWithin3Days
                               + trainStore_i_1$TimePriorWasSchoolHoliday
                               + trainStore_i_1$DaysWithinSchoolHoliday
                               + trainStore_i_1$Open
      )
      xreg_i_1 <- xreg_i_1[,-1]
      colnames(xreg_i_1) <- c("Mon", "Sat", "Sun", "Thur", "Tues", "Wed", 
                              "Promo", 
                              "SchoolHoliday", 
                              "ClosedDayPrior", 
                              "ClosingNextDay", 
      ...
      myAutoArima_0 <- auto.arima(ts(trainStore_i_0$LogSales), xreg = xreg_i_0, lambda = BoxCox.lambda(ts(trainStore_i_0$LogSales)))
      myAutoArima_1 <- auto.arima(ts(trainStore_i_1$LogSales), xreg = xreg_i_1, lambda = BoxCox.lambda(ts(trainStore_i_1$LogSales)))
    }
    ...
    if (section == 2) {
      predLogSales_i_0 <- data.frame(forecast(myAutoArima_0, xreg = newxreg_i, lambda = myAutoArima_0$lambda))
      predLogSales_i_1 <- data.frame(forecast(myAutoArima_1, xreg = newxreg_i, lambda = myAutoArima_1$lambda))
      predLogSales_i <- (predLogSales_i_0 + predLogSales_i_1)/2
    }
...
write.csv(output, file = "RossmannSimple3.csv", row.names = FALSE)
